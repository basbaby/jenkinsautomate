   ...
   stages {
        	...
        	steps {
					...
					container_Up = false
                    			timeout(time: 1, unit: "HOURS") { 
                        			sh "curl -u admin:admin -X GET -H \"Accept: application/json\" http://104.248.169.167:9000/api/qualitygates/project_status?projectKey=com.mycompany:jenkinsautomate > status.json"
      		...
      		steps {
        		script {
			  	//sh "docker stop apiops-anypoint-jenkins-sapi" 
			   	...
			   	LAST_STARTED = env.STAGE_NAME
				container_Up = false
			   	sh "docker build -t jenkinsautomate:mule -f Dockerfile ."
      ...
      stage("Run container") {
      		steps {
        		script {
			     	LAST_STARTED = env.STAGE_NAME
          		    	sh " docker run -itd -p 8082:8081 --name jenkinsautomate jenkinsautomate:mule"
				container_Up = true
				sh "sleep 60"
       			}
		}
     ...
     stage ("Munit Test"){
			...
			script {
				...
				publishHTML([allowMissing: false, alwaysLinkToLastBuild: false, keepAll: false, reportDir: "jenkinsautomate/target/site/munit/coverage", reportFiles: "summary.html", reportName: "Munit coverage Report", reportTitles: ""])
				}
			}		
        	}    
     }
 ...
 //   }
    ...
    stage("Archetype"){
			...
			script {
					...
					def data = "archetype.artifactId=generic-db-sapi"
		    			...
		    			sh "mvn -f jenkinsautomate/target/generated-sources/archetype/pom.xml -s $settings clean deploy -DaltSnapshotDeploymentRepository=nexus-snapshots::http://104.248.169.167:8081/repository/maven-snapshots/"
                  		} 
			}
              }   
     }
    ...
    stage("Deploy to Cloudhub"){
			...
			script {
				...
				sh "mvn -f pom.xml -s $settings package deploy -DmuleDeploy -DskipTests -Dkey=mymulesoft -Danypoint.username=njcdemo1 -Danypoint.password=Njclabs@123 -DapplicationName=jenkinsautomate-mule -Dcloudhub.region=us-east-2 -Danypoint.platform.client_id=2873c31e7152405e9dc38600007108e8 -Danypoint.platform.client_secret=70da4Da4228749B7A33C848E9a3C0849"
				}
			}
             	}
    }
    ...
    stage("Email") {
			...
			script {
        		    	...
        		    	emailext(subject: "$PROJECT_NAME - Build # $BUILD_NUMBER - $BUILD_STATUS!", body: "Build is Success.Please find the functional testing reports. In order to check the logs, please go to url: $BUILD_URL"+readFile("jenkinsautomate/emailTemplate.html"), attachmentsPattern: "cucumber-API-Framework/target/cucumber-reports/report.html", from: "${readProps["email.from"]}", mimeType: "${readProps["email.mimeType"]}", to: "${readProps["email.to"]}")
                        }
		}
    }   
    stage("Kill container") {
      		steps {
        		script {
	  	        	LAST_STARTED = env.STAGE_NAME		
          		    	sh "docker rm -f jenkinsautomate"
				sh "sleep 10"
				sh "docker rmi jenkinsautomate:mule"
